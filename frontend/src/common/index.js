...
export function getDueDateDay(dueDate) {
  const date = new Date(dueDate * 1000);
  return `${months[date.getMonth()]} ${date.getDate()}`;
}

export function getEventTime(d) {
  const date = new Date(d);
  return `${addZero(date.getHours())}:${addZero(date.getMinutes())}`;
}
export function getEventDate(d) {
  const date = new Date(d);
  return `${addZero(date.getDate())}.${addZero(date.getMonth()+1)}`;
}

export function getNormalDateTime(d) {
  const date = new Date(d);
...
export let instanceModal = null;
export function openModal(ConstructorModal, props = {}) {
  instanceModal = new ConstructorModal({
  ...
  document.getElementById('modal-wrap')
    .addEventListener('click', handleCloseModal, true);
...
export function getDateString(date) {
  return `${addZero(date.getDate())}.${addZero(date.getMonth()+1)}` + " " + `${addZero(date.getHours())}:${addZero(date.getMinutes())}`
}
